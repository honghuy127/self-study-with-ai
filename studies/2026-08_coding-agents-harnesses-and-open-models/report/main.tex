...
\title{Claude Code, Codex, and OpenCode\\
at the Source Level: Harness Architecture\\
and Open-Source Model Support\\
...
static source traces and official documentation snapshots; every stated
value carries a file:line or snapshot-line anchor at the pinned commits.
Part II asks what
...
their workflow. This decomposition has research pedigree. ReAct
interleaved reasoning traces with environment actions and showed the
combination beats either alone on question answering and interactive
tasks~\citep{yao2023react}. Toolformer
...
example surface~\citep{claudeCodePluginSurface,claudeCodeDocsPermissions}.

The \ccc{} core is closed source, and its pinned public checkout contains no
harness code~\citep{claudeCodePluginSurface}. We therefore treat documentation
...
\begin{itemize}
  ...
  \item four structured static-trace extractions (turn loops, tool
  surfaces, safety policies, and compaction parameters) in which every
  stated value carries a file:line or snapshot anchor
  (Section~\ref{sec:methods}, Sections~\ref{sec:turnloop} to~\ref{sec:safety});
...
(\ccc{})~\citep{codexRepo,opencodeRepo,claudeCodePluginSurface}. Official
...
rather than a commit~\citep{claudeCodeDocsHooks,claudeCodeDocsSubagents,%
claudeCodeDocsPlugins,claudeCodeDocsMcp,claudeCodeDocsMemory,%
claudeCodeDocsPermissions,claudeCodeDocsSandboxing,codexDocsSandboxing,%
...
contextual statements~\citep{minusXClaudeCodeTeardown,agiflowClaudeCodeInternals,%
tenguDecoded,claudeCodeRouterContext}.

\paragraph{Part I trace extractions.} Part I organized the extraction into
four traces: E1 turn loops, E2 tool surfaces, E3 safety policies, and E4
compaction parameters. Every stated value carries a \path{file:line} anchor
in the pinned checkouts or a line anchor in a documentation snapshot, and
the study notes carry those anchors one per claim. The anchors are the
evidence: a reader can re-open each cited file at the pinned commit and
check the value directly. One value, the \codex{} tool registration count,
was corrected from 33 to 32 during review against the pinned sources.
...
master~\citep{llamaCppServerDocs}. Part II was scoped as a briefing: its
12 notes (3 codebase, 8 documentation, 1 context) carry its evidence, with
every claim anchored the same way.
...
their own~\citep{claudeCodePluginSurface,agiflowClaudeCodeInternals,claudeCodeRouterContext}.
...
\centering
\small
\caption{Summary comparison. Anchors: E1 to E4 are the four trace
extractions; notes are the per-component study notes
...
blocks~\citep{claudeCodeDocsHooks}. The cadences attest that a loop with
...
open~\citep{claudeCodeDocsHooks,claudeCodePluginSurface}. Even discovery, where
...
connections~\citep{codexRepo,claudeCodeDocsMcp}.
...
parsed by a 19-line Lark grammar; \occ{} ports that grammar near-verbatim
(the port's comment reads ``Core types matching the Rust
...
observations~\citep{tenguDecoded,minusXClaudeCodeTeardown}.
...
and aimed at tool output, where a coding agent's context bloats:
...
not~\citep{claudeCodeDocsHooks,claudeCodeDocsMemory}. No numeric trigger,
...
runtime prefixes~\citep{codexRepo,codexDocsSandboxing}.
...
directly''~\citep{claudeCodeDocsPermissions,claudeCodeDocsSandboxing,%
claudeCodeDocsHooks}. The scope choice is deliberate and instructive:
...
arbitrary execution lives. Enforcement is platform-native
...
dropping egress outside an allowlist~\citep{claudeCodePluginSurface}. The
...
deserves~\citep{codexRepo,opencodeRepo,claudeCodeDocsPlugins}.
...
forks~\citep{claudeCodeDocsHooks,claudeCodeDocsPlugins,%
claudeCodeDocsSubagents}. The learning point is how much power the
...
core changes at all~\citep{claudeCodePluginSurface}. A good contract can replace
a plugin API.
...
first~\citep{claudeCodeDocsMemory}. The split explains the system's trust
...
lives~\citep{claudeCodeDocsSubagents,claudeCodeDocsHooks}.
...
devcontainer~\citep{claudeCodeDocsPermissions,claudeCodeDocsMcp,%
claudeCodePluginSurface}.
...
\begin{tikzpicture}[font=\scriptsize,
  proto/.style={draw=black!70, line width=0.7pt, rounded corners=1pt,
    fill=black!5, align=center, minimum width=3.65cm, minimum height=0.95cm,
    inner sep=2.5pt, text=black},
  reqlab/.style={draw=black!40, line width=0.5pt, rounded corners=1pt,
    align=center, minimum width=3.65cm, minimum height=0.85cm,
    inner sep=2.5pt, text=black},
  chip/.style={rounded corners=1pt, align=center, minimum width=3.65cm,
    minimum height=0.85cm, inner sep=2.5pt, text=black},
  rowhead/.style={draw=black!50, line width=0.5pt, rounded corners=1pt,
    fill=black!5, align=center, minimum width=2.1cm, inner sep=2.5pt,
    text=black},
  arr/.style={-{Stealth[length=1.8mm]}, draw=black!60, line width=0.5pt}]
% protocol columns
\node[proto] (p1) at (0,0) {\textbf{Chat completions}\\ plus tool calls};
\node[proto] (p2) at (4.05,0) {\textbf{Responses API}\\ (OpenAI)};
\node[proto] (p3) at (8.1,0) {\textbf{Anthropic}\\ \path{/v1/messages}};
% agent requirements
\node[reqlab, fill=genericcol!14] (oc) at (0,1.75) {\occ{} accepts any\\ chat-completions shape};
\node[reqlab, fill=nativecol!14] (cx) at (4.05,1.75) {\codex{} pins Responses;\\ \path{chat} is a hard error};
\node[reqlab, fill=unattcol!14] (ccreq) at (8.1,1.75) {\ccc{}: Anthropic protocol\\ only, as attested};
\draw[arr] (oc.south) -- (p1.north);
\draw[arr] (cx.south) -- (p2.north);
\draw[arr] (ccreq.south) -- (p3.north);
% Ollama row
\node[rowhead] at (-2.9,-1.4) {Ollama\\ 2024 snapshot};
\node[chip, fill=partcol!16] at (0,-1.4) {partial: chat yes, tools\\ listed as future};
\node[chip, fill=black!7] at (4.05,-1.4) {absent: not in snapshot};
\node[chip, fill=black!7] at (8.1,-1.4) {absent: not in snapshot};
% LM Studio row
\node[rowhead] at (-2.9,-2.5) {LM Studio\\ docs mirror};
\node[chip, fill=genericcol!14] at (0,-2.5) {supported: tools on chat\\ completions; no parallel flag};
\node[chip, fill=partcol!16] at (4.05,-2.5) {partial: streaming, effort,\\ prior state, remote MCP only};
\node[chip, fill=black!7] at (8.1,-2.5) {named only: shape\\ undocumented};
% llama.cpp row
\node[rowhead] at (-2.9,-3.6) {llama.cpp\\ reference server};
\node[chip, fill=genericcol!14] at (0,-3.6) {supported: tools via\\ jinja templates, JSON schema};
\node[chip, fill=partcol!16] at (4.05,-3.6) {partial: documented as a\\ converter to chat completions};
\node[chip, fill=genericcol!14] at (8.1,-3.6) {supported:\\ \path{/v1/messages} documented};
\end{tikzpicture}
...
not discovered until the request fails, or, worse, the model never
calls a tool and file editing quietly
...
% agents
\node[agent=nativecol] (cx) at (0, 2.8) {\textbf{Codex}\\ Responses wire};
\node[agent=genericcol] (oc) at (0, 0.0) {\textbf{OpenCode}\\ chat-completions wire};
\node[agent=unattcol] (cc) at (0,-2.8) {\textbf{Claude Code}\\ Anthropic protocol};
% servers
\node[server] (ol) at (8.6, 2.8) {\textbf{Ollama}\\ port 11434};
\node[server] (lm) at (8.6, 0.0) {\textbf{LM Studio}\\ port 1234};
\node[server] (gn) at (8.6,-2.8) {\textbf{Generic server}\\ llama.cpp class};
...
\draw[nat] (cx.east) to[out=-4, in=184]
  node[chip, pos=0.5, yshift=3pt, text=nativecol] {native; version\\ gate 0.13.4} (ol.west);
\draw[nat] (cx.east) to[out=-16, in=172]
  node[chip, pos=0.26, text=nativecol] {native; model\\ pull via lms} (lm.west);
\draw[nat] (cx.east) to[out=-30, in=158]
  node[chip, pos=0.24, text=nativecol] {config; Responses\\ wire only} (gn.west);
% OpenCode edges
...
\node[align=left, font=\scriptsize] at (0,-4.3)
...
\node[align=center, font=\scriptsize, text width=6.4cm] at (6.2,-4.3)
...
(minusXClaudeCodeTeardown~\citep{minusXClaudeCodeTeardown}), a binary-analysis catalog of
...
cross-verified by official documentation~\citep{agiflowClaudeCodeInternals}. None
...
behavior. Anchors make each value checkable at the pinned commit, but
checkability is not measurement. In Part II, the practical
