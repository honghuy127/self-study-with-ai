...
\begin{frame}{Three systems, three evidence tiers}
  ...
  \begin{itemize}
    \item Static source traces only: nothing executed, no API called, no
      ...
      data~\citep{codexRepo,opencodeRepo,claudeCodeSurface}.
    \item \ccc{} internals are boundaries, not findings; teardowns and
      routers enter only as hedged
      context~\citep{minusXTeardown,agiflowInternals,tenguDecoded,%
...
\begin{frame}{Limitations}
  \begin{itemize}
    ...
    shape remain open~\citep{claudeCodeSurface,claudeCodeModelDocs}.
...
\begin{frame}{Backup: evidence ledger}
  \begin{itemize}
    \item 44 registry entries: 20 codebase, 13 docs, 3 preprints, 4
      blog-tier context (teardowns and one community router); 45 notes
      including both per-part syntheses.
