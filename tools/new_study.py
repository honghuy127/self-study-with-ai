...
Usage: python3 tools/new_study.py <slug> --mode interactive|delegated|paper-reading
...
- mode is required: interactive (tutored mastery), delegated (agent-run
  investigation), or paper-reading (agent analysis of one target paper and
  a comprehensive presentation); there is no default
- interactive scaffolds learning/ and outputs/; delegated scaffolds report/,
  slides/, and reviews/ as the deliverables request; paper-reading scaffolds
  an anchored analysis, deck plan, slides, and reviews/
...
DEFAULT_DELIVERABLES = {
    ...
    "delegated": ["report", "slides"],
    "paper-reading": ["slides"],
}
DEFAULT_STATUS = {"interactive": "scoped", "delegated": "proposed", "paper-reading": "proposed"}
...
PAPER_READING_GATES_BLOCK = """gates:
...
def render_artifacts(fields: dict) -> str:
    ...
    if fields["mode"] == "paper-reading":
        lines.append("  paper_analysis: notes/_paper-analysis.md")
    if "report" in fields["deliverables"]:
        lines.append("  report: report/main.tex")
        lines.append("  pdf: report/build/main.pdf")
    if "slides" in fields["deliverables"]:
        lines.append("  deck_plan: slides/deck-plan.md")
        lines.append("  slides: slides/main.tex")
        lines.append("  slides_pdf: slides/build/main.pdf")
...
def render_study_yaml(template_text: str, fields: dict) -> str:
    ...
    block is swapped wholesale for interactive or paper-reading mode, and the
    experiments gate becomes boolean when the methodology runs experiments.
    The report_style line is dropped entirely when report is not a deliverable.
    """
    out = (
    ...
    elif fields["mode"] == "paper-reading":
        out = out.replace(DELEGATED_GATES_BLOCK, PAPER_READING_GATES_BLOCK)
...
def copy_templates(study: Path, study_id: str, title: str, config: dict) -> None:
    ...
    All modes keep sources/ and notes/ (the evidence packet); interactive
    adds learning/ and outputs/, delegated adds reviews/ plus report/ and
    slides/ as deliverables request, and paper-reading adds reviews/, an
    analysis template, and a deck plan. Experiments follow the methodology.
    """
    mode = config["mode"]
    ...
    if mode == "paper-reading":
        shutil.copy(TEMPLATES / "paper-analysis.md", study / "notes" / "_paper-analysis.md")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        choices=MODES,
        required=True,
        help="interactive mastery, delegated investigation, or target-paper analysis and presentation; required",
    )
    ...
    ap.add_argument(
        ...
        choices=INTENTS,
        default=None,
        help="defaults to understand (interactive/paper-reading) or survey (delegated)",
    )
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "comma-separated outputs; defaults to learning-note (interactive), "
            "report,slides (delegated), or slides (paper-reading)"
        ),
    )
    ap.add_argument(
        "--report-style",
    ...
    if mode == "paper-reading" and "slides" not in deliverables:
        print("new_study: paper-reading mode requires the slides deliverable", file=sys.stderr)
        return 2
    if mode == "paper-reading" and args.methodology in EXPERIMENTAL_METHODOLOGIES:
        print(
            "new_study: paper-reading mode supports source-only or static-code methodology; "
            "use delegated mode for experiments",
            file=sys.stderr,
        )
        return 2
    ...
    if mode == "interactive":
        ...
        print(f"next: fill studies/{study_id}/brief.md with the exact target paper and talk contract, then /read-paper studies/{study_id}")
    else:
