...
class RenderStudyYamlTest(unittest.TestCase):
    ...
    def test_paper_reading_mode_swaps_gates_block(self):
        out = render_study_yaml(
            self.template,
            fields(
                mode="paper-reading",
                intent="understand",
        ...
        self.assertIn("mode: paper-reading", out)
        for gate in ("paper_approved: false", "analysis_approved: false", "deck_approved: false"):
            self.assertIn(gate, out)
        self.assertNotIn("sources_approved", out)
        self.assertNotIn("experiments_approved", out)
        self.assertIn("paper_analysis: notes/_paper-analysis.md", out)
        self.assertIn("deck_plan: slides/deck-plan.md", out)

    def test_paper_reading_gates_block_matches_contract(self):
        self.assertIn("review_signed_off: false", PAPER_READING_GATES_BLOCK)
...
class CopyTemplatesTest(unittest.TestCase):
    ...
    def test_paper_reading_scaffold(self):
        study = self.scaffold(
            "2026-08_paper",
            mode="paper-reading",
            intent="understand",
        ...
        self.assertIn("analysis_approved: false", text)
        self.assertNotIn("experiments_approved", text)
...
class MainCliTest(unittest.TestCase):
    ...
    def test_modes_constant(self):
        self.assertEqual(MODES, ("interactive", "delegated", "paper-reading"))

    def test_paper_reading_requires_slides(self):
        self.assertEqual(
            new_study.main(["foo", "--mode", "paper-reading", "--deliverables", "decision-brief"]),
            2,
        )

    def test_paper_reading_rejects_experimental_methodology(self):
        self.assertEqual(
            new_study.main(["foo", "--mode", "paper-reading", "--methodology", "experimental"]),
            2,
        )
