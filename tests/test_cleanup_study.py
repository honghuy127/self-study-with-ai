...
class CleanupStudyTests(unittest.TestCase):
    ...
    def test_allows_paper_reading_mode(self) -> None:
        study = make_study(self.tmp, mode="paper-reading")
        cleanup_study.clean(study, dry_run=False)
        self.assertTrue((study / "slides" / "main.tex").is_file())
        self.assertFalse((study / "reviews").exists())
