...
class ZoneBasicsTests(unittest.TestCase):
    def test_all_agents_exist(self):
        for name in ("researcher", "summarizer", "paper-analyst", "experimenter", "writer", "reviewer"):
...
class PaperAnalystPermissionTests(unittest.TestCase):
    def test_can_write_only_paper_analysis(self):
        rules = edit_rules("paper-analyst")
        self.assertTrue(allows(rules, "studies/**/notes/_paper-analysis.md"))
        self.assertFalse(allows(rules, "studies/**/slides/**"))

    def test_no_web_access(self):
        permission = frontmatter("paper-analyst")["permission"]
        self.assertEqual(permission.get("webfetch"), "deny")
        self.assertEqual(permission.get("websearch"), "deny")
